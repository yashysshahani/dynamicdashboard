...
- debounce (boolean | number; default False):
    ...
    focus. If it's False, it will sent the value back on every change.
    If debounce is a number, the value will be sent to the server only
    after the user has stopped typing for that number of milliseconds.
